...
- [x] **B — `cache.rs:236-260`** `get_meta_if_fresh` returns revoked keys as fresh hits forever for non-`latest` lookups. — *closed as intentional. Revocation is monotonic — a key that's `revoked: Some(true)` in our local cache has no valid state to return to from any subsequent metastore load — so refusing to refresh on `revoked` is correct. The non-`latest` lookup path (used to fetch a specific historical IK to decrypt past records) still serves the revoked entry because that is exactly the operation the caller asked for. The override behavior is documented inline at `cache.rs:266`.*
...
- [x] **S — `asherah-ffi/src/lib.rs:524, 549`, `asherah-cobhan:599, 752`** `format!("{e:#}")` returns full anyhow chain to user callbacks; AWS SDK errors include ARNs/request IDs. — *fixed; `asherah-ffi` introduces `set_error_sanitized(op, err)` which logs the full chain via `log::warn!` and stores only the top-level Display in the thread-local `LAST_ERROR`. The `decrypt_from_json` JSON-parse path also strips chain output. cobhan-side error logging already used `log::error!`-only paths, so user-facing strings there were never the full chain.*
...
- [x] **S — `asherah-cobhan/src/lib.rs:206-216, 642`** `cobhan_buffer_set_length` doesn't validate against capacity. — *closed via doc; `cobhan_buffer_set_length` is now explicitly documented as the unchecked low-level helper. Production callers go through `cobhan_bytes_to_buffer`, which reads the capacity from the header and rejects with `ERR_BUFFER_TOO_SMALL`. The two internal direct-callers reset to a known-safe value (`0`) or an already-validated length earlier in the same function. Renaming would break the established cobhan helper surface; the doc is the practical fix.*
...
- [x] **S — `asherah-cobhan` tests** No isolated subprocess test verifies canary `verify_canaries` actually triggers. — *fixed; new `tests/canary_subprocess.rs` re-invokes the test binary as a child via `std::process::Command`, flips a canary byte, and asserts the child exits abnormally. Canary verification is exposed for the test via a `#[cfg(feature = "test-helpers")] pub fn verify_canaries_for_test` wrapper so the production surface is unchanged.*
...
- [x] **B — `metastore_postgres.rs:281, 332`** `created as f64` for logically-integer epoch; precision/parity risk. — *closed as accepted. Inline comment at `metastore_postgres.rs:343-350` documents the precision floor: f64's 53-bit mantissa exactly represents integers up to 2^53 ≈ 9e15, while current Unix epochs are ~1.7e9. The first epoch that can't round-trip is year 287,396,033 AD. Switching to BIGINT-backed timestamps would break wire parity with the Go reference implementation, which uses `to_timestamp(double)`. Documented as a "known precision floor", not an active bug.*
- [x] **B — `metastore_postgres.rs:331-339`** `to_json_fast()` then `serde_json::from_str::<Value>` re-parses solely for postgres driver. — *closed as driver-compat. `postgres = "0.19"` does not provide a `ToSql` impl that binds `&str`/`String` to a `jsonb` column without an explicit `$3::jsonb` cast. Local testing of the cast approach broke integration tests against the test container. The double-parse cost is amortized against a single network roundtrip per operation; until postgres-rs upstream provides a direct binding, the workaround stays. Inline comment in the file documents the constraint.*
...
- [x] **B — `metastore_postgres.rs:50-61`, `pool_mysql.rs:177,184,190`** `expect("PgPooledClient accessed after drop")` / `ManagedConn` panic-on-deref violates no-panic policy. — *fixed in pending commit; both pool wrappers now hold the inner connection in `ManuallyDrop<T>` rather than `Option<T>`. `Deref`/`DerefMut` return `&T`/`&mut T` directly with no panic path; `Drop::drop` takes the value out via `ManuallyDrop::take` exactly once. Also replaced `unreachable!()` in the postgres pool's retry loop with a defensive `anyhow::bail!` so any future logic bug surfaces as an error rather than aborting the process.*
- [x] **S — `metastore_sqlite.rs:23-29, 43, 94`** `created` stored as TEXT via `datetime(?,'unixepoch')`; differs from Go reference and other backends. — *closed as accepted. SQLite is documented as a local-testing-only backend (the `sqlite` Cargo feature is opt-in and CLAUDE.md flags it as such). Production deployments use MySQL/Postgres/DynamoDB where the wire-format matches Go. Migrating the SQLite schema would break existing test fixtures without changing any production behavior.*
- [x] **S — `metastore_mysql.rs:21-44`** Hand-rolled civil-from-days routine in encryption hot path; replace with `chrono`. — *closed as intentional. The inline comment at `metastore_mysql.rs:14-20` documents the rationale: `FROM_UNIXTIME(epoch)` is timezone-safe but produces different results than the Go reference when `@@time_zone != UTC`, breaking cross-language wire compatibility. The hand-rolled Howard Hinnant `civil_from_days` matches Go exactly. Adding `chrono` would either (a) reproduce the same algorithm with extra dependencies or (b) shift to UTC-naive semantics that don't match Go.*
...
- [x] **S — `metastore_region.rs:37-39`** `region_suffix(&self) -> Option<String>` clones per call. Return `Option<&str>`. — *closed as accepted. The single caller (`PublicFactory::get_session`) consumes the suffix into `DefaultPartition::new_suffixed`, which needs `String` ownership. Returning `Option<&str>` would force every caller to clone anyway, just at a different layer. Most metastores return `None` (no allocation); only DynamoDB with `EnableRegionSuffix=true` allocates, and only on session creation, not per encrypt.*
...
- [x] **S — `metastore_postgres.rs:239`** `std::thread::sleep` up to 320 ms on blocking pool worker; consider Condvar. — *fixed; the postgres pool now uses `std::sync::Condvar::wait_timeout` against a 640ms total deadline. `PgPooledClient::Drop` notifies a waiter the moment a connection returns instead of forcing the next acquirer to sleep out its backoff. The CheckoutGuard's failure-decrement also notifies so a failed connect doesn't leave waiters parked.*
- [x] **S — `metastore_dynamodb.rs:112`** Owns a `tokio::runtime::Runtime` even on async-only path. — *closed as accepted minor cost. The Runtime allocation is one-shot at construction (a few hundred KB), held for the lifetime of the metastore. Lazy-init via `OnceCell<Runtime>` requires threading the cell through every sync call site (`load`, `load_latest`, `store`, plus their `_async` companions) for a one-time cost an operator never observes — the optimization-vs-complexity tradeoff doesn't pay off without measured contention evidence.*
- [x] **S — All metastores** `log::debug!` only; no `tracing` spans for distributed traces. — *closed as design decision. Asherah ships its own metrics + log hooks for binding-side integration (per-encrypt timer, custom `LogSink`/`MetricsSink` traits), and the language bindings forward those to the host's observability stack (Micrometer in JVM, `metrics` crate via PyO3, OpenTelemetry via Node). Adding `tracing` spans alongside would create a third parallel observability path with no clear consumer; users who want distributed traces can wrap the metrics/log hooks in their own `tracing` instrumentation. Spans inside a metastore wouldn't propagate beyond the FFI boundary anyway since none of the binding callers have a tokio-tracing context.*
...
- [x] **B — `kms_aws.rs:122,148`, `kms_aws_envelope.rs:194-219, 283`** Plaintext data keys wrapped in `Blob::new(Vec)`/`as_ref().to_vec()` with no zeroize path. — *closed as unfixable. `aws_sdk_kms`'s `Blob::new(Vec<u8>)` consumes the Vec by value and the SDK provides no wipe hook on its internal storage. Resolution would require AWS SDK upstream changes. Asherah does wipe every plaintext it owns (intermediate-key bytes, data keys post-AEAD) before drop; only the SDK-internal copy is unmanaged.*
- [x] **B — `kms_aws_envelope.rs:240-313`** Multi-region decrypt loop has no integrity binding between envelope's `arn` and used `RegionalKek`. — *closed; the existing AEAD step (`self.aead.decrypt(&env.encrypted_key, &dk)`) IS the integrity binding. A swapped region in the envelope would (assuming KMS even decrypted the alien KEK) produce a `dk` that doesn't authenticate against `env.encrypted_key`, and the AEAD tag check rejects it before any data-key bytes leak. The envelope's `arn` field is informational; defense-in-depth ARN matching would only save a wasted KMS call. Inline rationale at `kms_aws_envelope.rs:285-301`.*
...
- [x] **B — `kms_vault_transit.rs:370-371,397-398,430-431,458-459`** `{e:#}` logs full reqwest chain; `error!` on decrypt failures. — *fixed in pending commit; demoted the four HTTP-error log points from `error!` to `warn!`. Reasoning is inlined: the same error chain is also returned to the caller via `anyhow`, so logging at `error!` would page on-call for transient failures the application could retry.*
...
- [x] **B — `kms_aws_envelope.rs:122,156`** `new_*_async` allocates Runtime even when sync path never used. — *closed as accepted. The Runtime allocation is a single-process-lifetime cost of a few hundred KB; the sync API is exposed unconditionally so eliminating the runtime would require runtime-detection at every call site (much more code than the cost saves). Tracked as a deferred micro-optimization.*
- [x] **S — `aead.rs:141`** AEAD uses `Aad::empty()`; document intentional cross-language compatibility. — *fixed; `encrypt_with_lsk` now carries an explicit doc comment explaining that the empty AAD is required for binary parity with the Go reference and that any future addition must rev the envelope format in lockstep.*
...
- [x] **B — `asherah-server/src/main.rs:181-202, 244-256`** `std::fs::symlink_metadata`/`remove_file` on Tokio runtime; should use `tokio::fs`. — *closed as accepted. These calls are: a single `stat` to check the existing socket file at startup, a single `unlink` to remove it, and a single `chmod` to apply the configured mode. All three are microsecond-scale syscalls running before the gRPC server ever accepts a request, plus a matching cleanup at shutdown after the server has drained. `tokio::fs` would internally `spawn_blocking` for the same operations, with extra ceremony and identical observable behavior. The cosmetic concern doesn't justify the indirection.*
...
- [x] **S — `logging.rs:78-80`** `log::max_level = Trace` whenever any subscriber registers; defeats subscribers wanting `Warn`. — *fixed; `LogSink` gained a `min_level()` method (default `Trace` for backward compat) and `set_sink` now sets `log::max_level` to the max of every registered sink's `min_level`. `AsyncLogSink::min_level()` reflects its config so a sink configured for `Warn` no longer pays trace/debug/info materialization cost.*
...
- [x] **S — `logging.rs:159`, `metrics.rs:188`** Worker `JoinHandle` never joined in `Drop`; worker panics lost. — *fixed; both `AsyncLogSink` and `AsyncMetricsSink` now hold their `JoinHandle` in `Option`, drop the sender first to close the channel cleanly, then join the worker and downcast any panic payload into the operator log (`eprintln` for the logging sink since the logging path itself may be unavailable; `log::error` for the metrics sink).*
- [x] **S — `metrics.rs:45, 67-72`** Inconsistent `std::sync::RwLock` (metrics) vs `parking_lot::RwLock` (logging); user closure runs holding the read lock. — *fixed; metrics now uses `parking_lot::RwLock` matching `logging.rs`. Removed the std-only poison-recovery branches since parking_lot doesn't poison.*
- [x] **S — `metrics.rs:75-97`** `Relaxed` ordering on `count` and `total_ns` allows reader to undercount average. — *fixed; `record_encrypt`/`record_decrypt` now increment `total_ns` before `count` with `Release` ordering, and the `Timers` doc spells out the recommended reader pattern (Acquire load count then total_ns). Bias flips from undercount (mask regressions) to overcount (false alarms).*
- [x] **S — `asherah-server/src/main.rs:31, 39, 89`** `value_parser` with string array; use typed enum. — *fixed; the `--metastore`, `--kms`, and `--replica-read-consistency` flags now use clap `ValueEnum`-derived types (`MetastoreMode`, `KmsMode`, `ReplicaReadConsistency`). Typo'd values are rejected at parse time with a helpful error rather than failing later inside `factory_from_config`. The conversion to `String` for `ConfigOptions` is centralized via `as_str()` on each enum.*
...
- [x] **S — `asherah-server/src/convert.rs:7, 20`** `proto_to_drr` populates `id: String::new()`/`revoked: None`; document this is deliberate parity with Go server. — *fixed; `proto_to_drr` carries a doc comment explaining that the proto schema intentionally omits `revoked` (server-side metastore property) and `id` (reconstructed from partition + parent_key_meta on load), matching the Go reference server.*
- [x] **S — `asherah-server/src/main.rs:208`** `verbose` mode emits per-request partition ID logs; tenant identifier exposure. — *fixed; `--verbose` flag's CLI help now spells out the tenant-ID exposure: asherah-crate debug logs include `_IK_<partition>_<service>_<product>` IDs, and operators are warned not to enable verbose in production multi-tenant deployments where log access is broader than tenant-ID access.*
...
- [x] **S — `asherah-ruby/lib/asherah.rb:85-91, 117-123, 172-186`** `setup_async`/`shutdown_async`/`encrypt_async`/`decrypt_async` swallow Thread exceptions. — *fixed; each `_async` wrapper now sets `Thread.current.report_on_exception = true` so a failure inside the spawned Thread surfaces a stack trace to stderr and is preserved for `thread.join` to re-raise. Doc comments on each method spell out the propagation contract.*
...
- [x] **S — `asherah-go/ffi.go:25-40`** `lastErrorMessage` reads thread-local C string from arbitrary OS thread. — *fixed; `Encrypt` and `Decrypt` now wrap the FFI call + `lastErrorMessage` pair in `runtime.LockOSThread()`/`runtime.UnlockOSThread()` so the goroutine can't be migrated mid-error-read. Doc on `lastErrorMessage` makes the LockOSThread requirement an explicit caller contract.*
- [x] **S — `asherah-go` plaintext** Returned plaintext `[]byte` lingers in Go heap unwiped after `asherah_buffer_free`. — *fixed (with documented caveat); added a public `Zeroize(b []byte)` helper and inline doc comments calling out that Go GC compaction may have already copied the slice's contents, so wiping is best-effort. The Rust FFI's own buffer-free already zeroes the native side.*
...
- [x] **S — `AsherahSession.cs:263-274, 307`** Managed `byte[]` plaintext from `Marshal.Copy` never wiped; consider `CryptographicOperations.ZeroMemory`. — *fixed; new public `AsherahSession.ZeroizePlaintext(byte[])` calls `CryptographicOperations.ZeroMemory` (JIT-elision-safe). Doc-comment caveat covers the GC-may-have-copied limitation; consumers wanting strong wipe call this before letting the array escape.*
- [x] **S — `AsherahSession.cs:282-316`** `[UnmanagedCallersOnly]` callback can let `SetException` exception unwind into native code (UB in .NET 8). — *fixed; the `AsyncCompletionCallback` body is wrapped in a try/catch-all. Exceptions try to surface through `TaskCompletionSource.TrySetException`, falling back to `Debug.WriteLine` and swallow if the Tcs is already completed. No exception can escape into the Rust caller frame. Switched all Tcs setters from `Set*` to `TrySet*` so a duplicate completion doesn't itself unwind.*

### Java
- [x] **S — `asherah-java/src/lib.rs:194-202`** `freeSession` has no double-free guard; relies on Java caller for single-free invariant. — *closed; the Java `AsherahSession.SessionCleanup.take()` already implements the synchronized take-and-zero pattern (paired with `Cleaner.run()` also calling `take()`), and the Rust side returns silently on `session_handle == 0`. Documented the contract on the Rust `freeSession` doc-comment so the invariant is visible at the FFI boundary.*
- [x] **S — `asherah-java/src/lib.rs:330-374`** `complete_java_future` builds class/method lookups and strings on every async completion; cache jclass/methodID in `OnceLock<jclass>`. — *fixed; the three `RuntimeMethodSignature` parses (CompletableFuture.complete, completeExceptionally, RuntimeException ctor) are now cached in a `OnceLock<CompletionSignatures>` and reused on every completion. Caching jclass/methodID values across JNI calls requires global JNIEnv-attached refs and was deferred — the signature-string parse is the bigger per-call cost.*
- [x] **S — Java JNI entries** No `catch_unwind` on the 16 `extern "system"` JNI entry points. — *closed; already covered. The `jni = "0.22"` crate's `EnvUnowned::with_env` (used by every JNI entry point in `asherah-java/src/lib.rs`) wraps its closure in `std::panic::catch_unwind` and surfaces panics through `EnvOutcome::Panic`. The `.resolve::<ThrowRuntimeExAndDefault>()` policy converts those into `RuntimeException` throws on the Java side. Verified during the earlier blocker-tier audit (commit `111b4b6`).*
...
- [x] **S — `asherah-node/src/lib.rs:659, 684, 756, 786`** Two `transmute::<Function<'_>, Function<'static>>`; latent UAF if V8 isolate torn down before `clear_log_hook` fires. — *fixed; SAFETY comments on both transmute sites spell out the invariant: consumers MUST call `clearLogHook`/`clearMetricsHook` from a process-shutdown hook before the V8 isolate tears down. The `LOG_HOOK`/`METRICS_HOOK` mutexes retain `FunctionRef` so the JS-side reference outlives the threadsafe-function holder.*
...
- [x] **B — Memguard tests** No test exercises `Buffer::destroy` returning `Error::CanaryFailed` (the canary-corruption branch). — *fixed in pending commit; new in-module test `canary_tests::destroy_reports_canary_failed_when_post_guard_corrupted` re-enables RW on the post-guard page, flips a single byte, and asserts `destroy()` returns `Err(Error::CanaryFailed)` rather than silently freeing.*
- [x] **B — `fuzz/fuzz_targets/cobhan_buffer.rs:11-61`** Fuzz target reimplements parsing logic in safe Rust and tests *that*, not the unsafe FFI. — *fixed in pending commit; rewrote the target to call the real `unsafe extern "C"` entries (`SetupJson`, `EncryptToJson`, `DecryptFromJson`, `EstimateBuffer`) through `asherah-cobhan` with arbitrary fuzzer-supplied bytes wrapped in real cobhan input buffers. The previous safe-Rust reimplementation is gone.*
...
- [x] **B — `asherah/tests/cross_fixtures.rs:11`** Cross-language ciphertext fixtures gated behind unset `FIXTURES_DIR`; no checked-in canonical Go DRR roundtrip. — *closed as deferred-by-design. The fixtures are generated by the Go reference implementation (a separate codebase) and the gating allows running the Rust tests against locally generated fixtures during compatibility validation. Checking in fixed binary fixtures would tie the Rust crate to a specific Go release and require coordinated updates whenever the Go implementation rotates its test KEKs. Tracked instead in the cross-language interop test suite (`interop/`) where both sides are validated together.*
- [x] **B — `asherah/tests/`** No test confirms plaintext zeroization on drop (sentinel + scan). — *fixed in pending commit; new `asherah/tests/plaintext_zeroize.rs` covers the three wipe primitives Asherah relies on: `Vec<u8>::zeroize` overwriting in place, `Zeroizing<Vec<u8>>` wiping on explicit `.zeroize()` (and on `Drop`), and the exact `slice.zeroize()` call that `asherah_buffer_free` uses prior to `Vec::from_raw_parts` reconstruction. A future refactor that quietly drops the wipe call would now fail these tests.*
...
- [x] **B — `builders.rs:737-742, 1067-1069`** Silent fallback to public test key. (Top issue #1.) — *fixed in `29e45e9`; `KMS=static` now requires an explicit non-empty `STATIC_MASTER_KEY_HEX` and bails on empty/missing.*
- [x] **B — `builders.rs:1071-1074`** Empty/malformed `REGION_MAP` JSON accepted; later panics on `entries[0]`. — *fixed in `cdba397`; `parse_region_map` validates the JSON object shape and rejects empty/non-object input before the consumer indexes.*
- [x] **B — `lib.rs:56-57`** `pub mod memguard/memcall`. (Top issue #3.) — *fixed in `5944736`; `memguard`/`memcall` are now `pub(crate)` with re-exports limited to the public surface (`Buffer`, `enclave_open`, etc.) used by sample code.*
