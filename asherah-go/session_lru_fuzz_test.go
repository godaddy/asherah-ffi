...
//   - the list never exceeds maxSize
...
func FuzzSessionLRU(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, ops []byte, maxSizeByte uint8) {
		...
		if maxSize < 1 {
			maxSize = 1 // acquireSession's real callers never allow maxSize < 1.
		}
		...
		for step, b := range ops {
			...
			if _, ok := lruTouch(cache, lru, key); !ok {
				...
				lruInsertEvict(cache, lru, maxSize, sessionCacheEntry{partition: key, session: sess})
			}

			assertLRUInvariants(t, cache, lru, maxSize, step)
		}
	})
}
