...
fn encrypt_bytes(partition_id: &str, data: &[u8]) -> PyResult<String> {
    ...
    // in-memory AEAD op; releasing/reacquiring the GIL around it collapses
    // multi-threaded throughput (~3-4x slower at 4-8 threads, from GIL
    // handoff thrashing on a sub-microsecond call). A cache miss that hits
    // the metastore/KMS is rare and briefly holds the GIL, matching pre-T5
    // behavior; callers needing concurrency across those should use
    // encrypt_bytes_async, which runs on the tokio runtime off the GIL.
    // Supersedes the GIL release added for T5 in
    // docs/review-2026-05-05-findings.md.
    let drr = session.encrypt(&input).map_err(anyhow_to_py)?;
...
fn decrypt_bytes<'py>(
    ...
    let bytes: Zeroizing<Vec<u8>> = Zeroizing::new(session.decrypt(drr).map_err(anyhow_to_py)?);
...
fn decrypt_string(partition_id: &str, data_row_record: &str) -> PyResult<String> {
    ...
    let drr: ael::types::DataRowRecord = serde_json::from_str(data_row_record)
        .map_err(|e| PyRuntimeError::new_err(format!("invalid DataRowRecord JSON: {e}")))?;
    // Hold the GIL across the decrypt — see `encrypt_bytes` for why the sync
    // hot path must not release it.
    let bytes: Zeroizing<Vec<u8>> = Zeroizing::new(session.decrypt(drr).map_err(anyhow_to_py)?);
    ...
    // skipping the wipe.
    std::str::from_utf8(&bytes).map_err(|e| PyRuntimeError::new_err(format!("utf8 error: {e}")))?;
...
impl PySession {
    pub fn encrypt_bytes(&self, data: &[u8]) -> PyResult<String> {
        ...
        let drr = self.inner.encrypt(&input).map_err(anyhow_to_py)?;
...
impl PySession {
    fn decrypt_raw(&self, data_row_record: &str) -> PyResult<Zeroizing<Vec<u8>>> {
        ...
        // `encrypt_bytes` for the rationale.
        let pt = self.inner.decrypt(drr).map_err(anyhow_to_py)?;
        Ok(Zeroizing::new(pt))
    }
}
